...
func TestNegativeValidation(t *testing.T){
	log.Printf("Astrix goes for War - Pass bad responses")
	ctx, cancel := context.WithTimeout(context.Background(), time.Second)
	defer cancel()
	conn, err := grpc.DialContext(ctx,"bufnet", grpc.WithDialer(bufDialer),grpc.WithInsecure())
	if err != nil {
	...
	defer conn.Close()
	c := pb.NewSearchServiceClient(conn)
		r, err := c.Search(ctx, &pb.SearchRequest{Query: "1Protocol Buffer",EmailId: "alex.test##gmail.com"})
	if err != nil {
		t.Fatalf("could not execute search: %v", err)
	}
	if r.SearchResponse != "Invalid Inputs" {
		t.Fail()
		log.Printf("Expected Server response to be invalid when invalid data is passed")
	}
